...
const Results = (props) => {
    ...
    const _handleAddLibrary = async (id, title, author) =>{
            ...
            <GridList className={classes.gridList} cols={4} cellHeight={300} spacing={16}>
                {results.map((result) => {
                    ...
                    <GridListTileBar
                        ...
                        titlePosition ={'top'}
                        actionIcon={
                            <IconButton aria-label={`${result.id[0]._}`} onClick={() => _handleAddLibrary(result.id[0]._, result.best_book[0].title, result.best_book[0].author[0].name[0])}>
